...
for week in range(1, 53):
    for day in range(1, 8):
        ...
        day_sales = {}
        if (week < 13):
            average_temperature = round(random.gauss(37, 10))
        elif (week < 26):
            average_temperature = round(random.gauss(60, 10))
        elif (week < 39):
            average_temperature = round(random.gauss(80, 10))
        else:
            average_temperature = round(random.gauss(50, 10))
        for _ in range(number_sold):
            ...
            item_sold.average_temperature = average_temperature
            # Food item or drink
            ...
            # Fall
                ...
                # Food item distribution
                    ...
                    if (rand <= .20):
                        ...
                        item_sold.item_id = 'Ice Cream Sundae'
            #print(item_sold)
            if (not item_sold.item_id in day_sales):
                day_sales[item_sold.item_id] = 1
            else:
                day_sales[item_sold.item_id] += 1

        for key in day_sales:
            restaurant_sales.append({"Item_id": key, "Week Number": week, "Weather Condition": weather_condition, "Average Temperature": average_temperature, "Day of Week": day, "Quantity": day_sales[key]})
        day_sales.clear()
